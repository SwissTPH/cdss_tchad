...
function getConditionValue(contact, report, condition){
  if(getField(report, condition)){
    const condition_value = getField(report, condition).valueOf();
    return parseInt(condition_value);
  }
}
...
function hasCondition(contact, report, condition) {
  if (getField(report,condition)){
    ...
    return has_condition ==='1' || has_condition.toLowerCase() ==='yes';
  }else {return false;}
}
